...
namespace RoboCup
{
    public class DefenderTop : Player
    {
        private const int WAIT_FOR_MSG_TIME = 10;

        public DefenderTop(Team team, ICoach coach)
            : base(team, coach)
        {
        ...
        void SearchBall()
        {
            while(true)
            {
                ...
                if (CheckBallInMyHalf(coachBall) &&  ball_obj.Distance.Value < 40 )
                {
                    return;
                }
            }
